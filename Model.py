...
class Model(nn.Module):
	...
	def forward_MM(self, adj, image_adj, text_adj, alpha = 0.5, audio_adj=None, audio_adjuiui=None):
		if args.trans == 0:
			image_feats = self.leakyrelu(torch.mm(self.image_embedding, self.image_trans))
			text_feats = self.leakyrelu(torch.mm(self.text_embedding, self.text_trans))
		elif args.trans == 1:
		...
		embedsImageAdj = torch.spmm(image_adj, embedsImageAdj)# 16 u
		...
		return embeds[:args.user], embeds[args.user:]# U I embeds

	def forward_cl_MM(self, adj, image_adj, text_adj, alpha = 0.5, audio_adj=None, audio_adjuiui=None):
		#image_adj = alpha * image_adj +(1-alpha) * image_adjuiui
		#text_adj = alpha * text_adj + (1-alpha) * text_adjuiui
		if args.trans == 0:
			image_feats = self.leakyrelu(torch.mm(self.image_embedding, self.image_trans))
			text_feats = self.leakyrelu(torch.mm(self.text_embedding, self.text_trans))
		elif args.trans == 1:
